...
const useValidateEmail = () => {
    ...
    const isEmailInvalid  = useMemo((): boolean => {
      ...
      const isInvalidLength = valueEmail.length < minLengthEmail || valueEmail.length > maxLengthEmail; 
      const isInvalidLengthDomain = getLengthDomain < minLengthDomain || getLengthDomain > maxLengthDomain;
      const isInvalidLengthUserNameEmail = getLengthNameEmail < minLengthUserNameEmail || getLengthNameEmail > maxLengthUserNameEmail;

      return isInvalidLength || isInvalidLengthUserNameEmail || isInvalidLengthDomain || !valueEmail.includes("@") || !valueEmail.includes(".");
    }, [valueEmail]);

    const onValidateInputEmail = useCallback((e: FocusEvent<HTMLInputElement> | KeyboardEvent<HTMLInputElement>) => {
      ...
      const hideErrorInputEmail = useCallback((e: FocusEvent<HTMLInputElement> | KeyboardEvent<HTMLInputElement>) => {
        if (!isEmailInvalid) {
      ...
      return {valueEmail, isFirstRenderEmail, isEmailValidation, warningMessageEmail, setValueEmail, hideErrorInputEmail, onValidateInputEmail}
};
